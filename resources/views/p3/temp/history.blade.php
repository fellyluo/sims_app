...
@section('content')
<div class="space-y-5">
    <a href="{{ route('p3.temp.index') }}" class="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-primary"><i data-lucide="arrow-left" class="w-4 h-4"></i> Kembali ke Pengajuan</a>
    <div>
        <h1 class="page-title">{{ $status === 'approve' ? 'Riwayat Disetujui' : 'Riwayat Ditolak' }}</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Pengajuan P3 yang sudah diproses</p>
    </div>
