@extends('layouts.app')
@section('title', 'Tambah P3 — '.$siswa->nama)

@section('content')
<div class="max-w-xl mx-auto space-y-4" x-data="p3Form(@js(route('p3.kategori.json')))" x-init="init()">
    <a href="{{ route('p3.siswa.show', $siswa) }}" class="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-primary"><i data-lucide="arrow-left" class="w-4 h-4"></i> Kembali</a>
    <div>
        <h1 class="page-title">Tambah P3</h1>
    ...
    <form method="POST" action="{{ route('p3.siswa.store', $siswa) }}" class="card p-6 space-y-4">
        @csrf
        <div>
