@extends('layouts.app')
@section('title', 'Ajukan Poin Siswa')

@section('content')
<div class="space-y-5">
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
            <h1 class="page-title">Ajukan Poin Siswa</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Pilih siswa untuk mengajukan poin — menunggu persetujuan kesiswaan</p>
        </div>
        <a href="{{ route('poin.guru.riwayat') }}" class="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition"><i data-lucide="history" class="w-4 h-4"></i> Riwayat Saya</a>
    </div>
    ...
    <form method="GET" class="card p-4 flex flex-wrap gap-2 items-center">
        ...
        @if(request('search'))
        <a href="{{ route('poin.guru.index') }}" class="px-4 py-2 rounded-xl text-sm text-slate-500 hover:text-slate-700 transition">Reset</a>
        @endif
    </form>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        @foreach($siswas as $s)
        <a href="{{ route('poin.guru.create', $s) }}" class="card p-3.5 flex items-center gap-3 hover:border-primary/40 transition">
