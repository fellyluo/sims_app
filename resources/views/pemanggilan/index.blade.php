@extends('layouts.app')
@section('title', 'Pemanggilan Ortu/Siswa')
...
<div class="space-y-5">
    <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Rekapan pemanggilan orang tua/siswa beserta permasalahan dan hasilnya</p>
        </div>
    ...
    <form method="GET" class="card p-4 flex flex-wrap gap-3 items-end">
        ...
        <div class="flex-1 min-w-48">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Ketik nama..." class="form-input">
        </div>
        ...
        @if(request()->hasAny(['kelas','dari','sampai','search']))
        <a href="{{ route('pemanggilan.index') }}" class="px-4 py-2.5 rounded-xl text-sm text-slate-500 hover:text-slate-700 transition">Reset</a>
        @endif
    </form>
            ...
            <table class="data-table w-full">
                <thead>
                    <tr>
                        ...
                        <th>Kelas</th>
                        <x-sortable-th field="dipanggil" label="Dipanggil" />
                        <th>Perihal</th>
                        <th>Status</th>
                        <th>Dicatat Oleh</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span class="badge bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300">{{ \App\Models\Pemanggilan::DIPANGGIL[$p->dipanggil] ?? $p->dipanggil }}</span></td>
                        ...
                        <td class="text-xs text-slate-400">{{ $p->pencatat?->displayName() ?? '-' }}</td>
                        <td>
                    ...
                    @empty
                    <tr><td colspan="9" class="text-center text-slate-400 py-8">Belum ada catatan pemanggilan.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
