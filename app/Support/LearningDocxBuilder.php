...
final class LearningDocxBuilder
{
    ...
    public static function write(string $path, array $doc): bool
    {
        ...
        $zip->addFromString('word/document.xml', self::documentXml($doc));
    ...
    private static function run(string $text, bool $bold = false, bool $italic = false, int $sz = 20): string
    {
    ...
    /** @param array{align?:string,after?:int,before?:int,left?:int,hanging?:int} $opts */
    private static function p(array $runs, array $opts = []): string
    {
        $pPr = '';
    ...
    private static function tc(array $content, array $opts): string
    {
    ...
    private static function tr(string $cells): string
    {
        return '<w:tr>'.$cells.'</w:tr>';
    }

    /** @param int[] $gridCols */
    private static function tbl(array $gridCols, string $rows, bool $bordered = true, bool $trailingP = true): string
    {
        $borders = $bordered
            ? '<w:tblBorders>'
